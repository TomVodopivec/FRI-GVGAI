...
public final class Metrics {
    ...
    public static final int WIN = 0;            //average percentage of wins (higher better)
    public static final int WIN_SCORE = 1;      //average score for wins (higher better)
    public static final int LOSE_SCORE = 2;     //average score for non-wins (higher better)
    public static final int WIN_STEPS = 3;      //average number of steps for wins (lower better)
    public static final int LOSE_STEPS = 4;     //average number of steps for non-wins (higher better)
    public static final int NUM_ITERS = 5;      //average number of MCTS iterations per move (per 40ms)
    public static final int NUM_FORWARDS = 6;   //average number of calls to advance() (forward model) per move (per 40ms) (higher better)
    public static final int TIME_INIT = 7;      //average initialization time in miliseconds
    ...
    public static final String[] LABELS =
            {
                    ...
                    "WinSteps",
                    "LoseSteps",
                    "MCTSiter",                 //normalized per moves
                    "NumForwards",              //normalized per moves
                    "InitTime",
            };

    public static final String[] LABELS_SIXC =
            {
                    "Win[%%]",
                    "WinScr",
                    "LosScr",
    ...
    public static final int NUM_FILES = 6;
    public static final int F_RAW = 0;
    public static final int F_LEVELS = 1;
    public static final int F_GAMES = 2;
    public static final int F_SUMMARY = 3;
    public static final int F_SHORT = 4;
    public static final int F_RAWVER = 5;
        ...
        try {
            for (int m = 0; m < NUM_METRICS; m++) {
                if (!printIgnoreMetrics[m]) {
                    printer.format(FORMATS[m], resultsSummary[m].getMean());
                    if (detail > 0) {
                        printer.format(" " + FORMATS[m], resultsSummary[m].getConf());
                    }
                    if (detail > 1) {
    ...
    public static void printByGames(PrintStream printer, int detail)
    {
        try {
            for (int g = 0; g < resultsLevels.length; g++) {
                for (int m = 0; m < NUM_METRICS; m++) {
                    if (!printIgnoreMetrics[m]) {
                        printer.format(FORMATS[m], resultsGames[g][m].getMean());
                        if (detail > 0) {
                            printer.format(" " + FORMATS[m], resultsGames[g][m].getConf());
                        }
                        if (detail > 1) {
        ...
        try {
            for (int g = 0; g < resultsLevels.length; g++) {
                for (int l = 0; l < resultsLevels[g].length; l++) {
                    for (int m = 0; m < NUM_METRICS; m++) {
                        if (!printIgnoreMetrics[m]) {
                            printer.format(FORMATS[m], resultsLevels[g][l][m].getMean());
                            if (detail > 0) {
                                printer.format(" " + FORMATS[m], resultsLevels[g][l][m].getConf());
                            }
                            printer.format("  ");
                        }
                    }
    ...
    public static void printLastResults(PrintStream printer)
    {
        try {
            for (int m = 0; m < NUM_METRICS; m++) {
                if (!printIgnoreMetrics[m]) {
                    printer.format(FORMATS[m], lastResults[m]);
                    printer.format("  ");
                }
            }
            printer.format("  ");
